...
/**
 * Integration tests for the Enhanced Organization Authentication feature.
 *
 * <p>Verifies the authorization code flow for a sub-organization user logging in via a root
 * organization application that has {@code enhancedOrgAuthenticationEnabled} set to {@code true}.
 * All OAuth2 requests use the o-path URL format {@code /t/<tenant>/o/<orgId>/...}, which
 * bypasses the organization discovery UI and routes directly to the sub-organization context.</p>
 */
public class EnhancedB2BLoginTestCase extends OAuth2ServiceAbstractIntegrationTest {
    ...
    /**
     * Builds the o-path URL ({@code /t/<tenant>/o/<orgId>/...}) for the given endpoint.
     * Forces the tenant prefix even for carbon.super, which {@code getTenantQualifiedURL} skips.
     */
